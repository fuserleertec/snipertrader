...
`live_trading` is always `false`. Refreshed on startup and every 15m.

Score inputs on `/top` (DE-owned, not FE display names): `volume`,
`volatility`, `session_active`, `levels_available`, `pattern_count`.
