...
class SetupOrchestrator:
    ...
    async def _attach_ensemble(self, cand: SetupCandidate) -> None:
        vwap = await get_session_vwap(self.store, cand.symbol)
        score, parts = score_ensemble(
            conviction=cand.conviction,
            contributing_factors=cand.contributing_factors,
            kill_zone_aligned=cand.kill_zone_aligned,
        ...
        cand.ensemble_score = score
        cand.rank_components = parts
    ...
    async def submit(self, incoming: list[SetupCandidate]) -> list[SetupCandidate]:
        ...
        for cand in winners:
            ...
            if cand.conviction < min_conv:
                ...
                continue
            await self._attach_ensemble(cand)
