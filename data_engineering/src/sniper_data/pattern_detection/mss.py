...
class _Sym:
    ...
    fresh_swings: list[SwingPoint] = field(default_factory=list)
...
class MSSDetector:
    ...
    def take_swings(self, symbol: str) -> list[SwingPoint]:
        """Drain swing highs/lows confirmed since the last call (Phase 2 anchors)."""
        st = self._state[symbol]
        out = list(st.fresh_swings)
        st.fresh_swings.clear()
        return out
    ...
    def _confirm_swings(self, st: _Sym, latest_index: int) -> None:
        ...
        if all(cand.high > b.high for i, b in enumerate(window) if i != n):
            point = SwingPoint("high", cand.high, cand.close_ts_ms, cand_index)
            st.swings.append(point)
            st.fresh_swings.append(point)
            self._note_swing(st, "high", cand.high)
        if all(cand.low < b.low for i, b in enumerate(window) if i != n):
            point = SwingPoint("low", cand.low, cand.close_ts_ms, cand_index)
            st.swings.append(point)
            st.fresh_swings.append(point)
