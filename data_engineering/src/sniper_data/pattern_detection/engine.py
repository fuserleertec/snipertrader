...
from sniper_data.pattern_detection.anchors import publish_anchor, swing_to_registration
...
class PatternBatch:
    ...
    anchors: list[AnchorRegistration] = field(default_factory=list)
...
class PatternEngine:
    ...
    async def on_bar(self, bar: OHLCVBar) -> PatternBatch:
        ...
        for swing in self.mss.take_swings(bar.symbol):
            req = swing_to_registration(bar.symbol, swing, bar.asset_class)
            await self._emit_anchor(req)
            batch.anchors.append(req)
    ...
    async def _emit_anchor(self, req: AnchorRegistration) -> None:
        await publish_anchor(self.bus, req)
