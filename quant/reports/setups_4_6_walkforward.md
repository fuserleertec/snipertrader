...
Dormant `mss_break` / `order_block` / `sweep_mss` / `ob_fvg` are **not**
in the validate enum. Setup 2 overlaps publish as `fvg_entry` only —
no `ob_fvg` accept path or alias on `/risk/validate` (PR #9 E2E alias
is ignored here).
