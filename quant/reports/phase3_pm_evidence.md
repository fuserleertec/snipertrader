...
|---|---|---|---|
| 1 | Backtest Setups 4–6 | **PASS** | [`quant/reports/setups_4_6_walkforward.md`](setups_4_6_walkforward.md). Enum excludes `mss_break` / `order_block` / `sweep_mss`. Aligned to PR #9 product keys. |
...
| Setup | `setup_type` | Product key | WF OOS n | WF OOS win | Baseline full n |
|---|---|---|---:|---:|---:|
| 4 | `sd_extension_fade` | `4_sd_extension_fade` | 3 | 0% | 10 |
| 5 | `vwap_pullback_cont` | `5_vwap_pullback_cont` | 1 | 100% | 2 |
| 6 | `avwap_ob_confluence` | `6_avwap_ob_confluence` | 14 | 7.1% | 56 |
...
- Performance: `GET /performance/summary` → `by_setup` keys `1_liquidity_sweep_vwap_reclaim` … `6_avwap_ob_confluence`; each bucket has `setup_type`, `sharpe_ratio`, `max_drawdown_pct`. Drift: rolling 20-trade WR &lt; 45% → `drift_warning`.
