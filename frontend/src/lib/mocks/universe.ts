...
/** Publish-only ranking — component max weights 40/20/15/15/10, ensemble_score 0–100. */
export function mockRanking(symbol: string, setup: SetupType, seq: number): {
  ...
  return { ensemble_score: sumRankComponents(rank_components), rank_components };
}
...
function signalOf(
  ...
  return {
    ...
    exit_price: null,
    closed_ts_ms: null,
    category: inferAssetClass(symbol),
