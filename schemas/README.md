...
|---|---|---|
| `GET /v1/universe/top?limit=10\|20` | [`universe_top.schema.json`](universe_top.schema.json) | Locked DE ranked book. Quant cut over: limit=10 → P0 ensemble, limit=20 → categorized + history. Fallback only if DE unreachable. |
...
`setup_type` (locked, six values): `sweep_reclaim` · `fvg_entry` · `po3_judas` · `sd_extension_fade` · `vwap_pullback_cont` · `avwap_ob_confluence`. Dormant (`mss_break`, `order_block`, `sweep_mss`, `ob_fvg`) → 422. Walk-forward S4–S6 uses those three live names only. Optional publish-only on `setup_signals` and Signal API responses: `contributing_factors` (`string[]`) and `factor_breakdown` (`{name, weight, score, note?}[]`) — not on `/risk/validate`.
